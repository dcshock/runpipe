...
// BuildOptions configures how a pipeline is built from config (e.g. retry persist).
...
func BuildPipeline(reg *Registry, cfg *PipelineConfig, opts *BuildOptions) (*pipeline.Pipeline, error) {
	...
	if len(cfg.Stages) == 0 {
		return &pipeline.Pipeline{Name: cfg.Name, Stages: nil}, nil
	}
	...
	return &pipeline.Pipeline{Name: cfg.Name, Stages: stages}, nil
}
